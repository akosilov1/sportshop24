...
?><h1>Коэффициенты для цен</h1>
    ...
    <p><button class="set_coeff_all">Выставить для всех</button> > <input type="text" value="" class="coeff_all"/>   </p>
...
while($rez = $res->fetch_assoc()){
    ?>
        ...
        <tr data-id="<?=$rez["cat_id"]?>" data-parent="<?=$rez["parent"]?>">
            ...
            <td><?=$rez["name"]?></td>
            <td><?=$rez["parent"]?></td>
            <td><input class="coeff" type="text" name="cat[<?=$rez["id"]?>]" value="<?=$rez["coeff"]?>"/></td>
        </tr>
...
<?}?>
    ...
    <script>
    	...
    	$(".set_coeff_all").on('click', function(e){
    	    e.preventDefault();
    	    var all_coeff = $(".coeff_all").val();
    	    $(".coeff").val(all_coeff);
        })
    </script>
<?