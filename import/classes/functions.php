<?
class Functions{
	private static $db, $pdo,$db_prefix;
	static function init($config){
        ...
        self::$db->error;
        self::$db_prefix = $config['table_prefix'];
	}
    static function GetCategoryForName($name){

		$q = 'SELECT category_id FROM '.self::$db_prefix.'category_descriptions WHERE category=\''.$name.'\'';
...
//		print_r($ar_features);
    ...
    static function GetFeaturesFromName($name){
        global $config;
        $q = "SELECT feature_id FROM ".self::$db_prefix."product_features_descriptions WHERE description='".$name."'";
    ...
    static function GetFeatureVariantFromName($id, $name){
        $q = 'SELECT
            ...
            v.feature_id
            FROM
            '.self::$db_prefix.'product_feature_variants AS v
            INNER JOIN '.self::$db_prefix.'product_feature_variant_descriptions AS vd ON v.variant_id = vd.variant_id
    ...
    static function GetOptionsVariantIdFromName($id, $name){
        $q = "SELECT vd.variant_id 
            FROM ".self::$db_prefix."product_option_variants_descriptions AS vd
            INNER JOIN ".self::$db_prefix."product_option_variants AS ov ON vd.variant_id = ov.variant_id
        ...
        if(count($get_prod->products) > 0){
            ...
            if(!$item->active) {
                ...
                /*  A for Active
                    D for Disabled
                    H for Hidden
                Если товара нет - деактивируем
                */
                echo "Товар ($prod_id) не активен \n\t< Деактивируем >\n";
                return $mag->put_product($prod_id, array("status" => "D"));
            }else{
                echo "\t< Активируем товар > \n";
                // Активируем товар
                $mag->put_product($prod_id, array("status" => "A"));
                if(is_array($item->size)){
                    // Есть размеры
                    echo "ISSET SIZES\n";
                    if(!$get_prod->products[0]->has_options){
                        ...
                        pre_print($s_coeff);
                        echo "CAT_ID ".$get_prod->products[0]->category_ids[0]."\n";
                    ...
                    echo "PRICE ".$i_price."\n";
                    return $mag->put_product($prod_id,array(
                        "price"     => $i_price,
                        "amount"    => $i_quantity,
                        "status"    => "A"
                    ));
            ...
            $f_ = fopen(__DIR__.'/update_prod_add_log.txt','a');
            ob_start();
    ...
    private static function getImages($images){
        $ar_img = array();
        array_map('unlink', glob(__DIR__."/../images/copy/*.jpeg"));
    ...
    static function prepareImages($ar_images){
        $ar_images = self::getImages($ar_images);

        $f = fopen(__DIR__.'/images.log','a');
        fwrite($f,date("d_m h:i:s ").print_r($ar_images, true));

        array_map('unlink', glob(__DIR__."/../images/*.png"));
        array_map('unlink', glob(__DIR__."/../images/*.jpeg"));
        array_map('unlink', glob(__DIR__."/../images/*.jpg"));

        $rez = array();
        foreach ($ar_images as $image){
            fwrite($f,"START IMG>>> ".$image."\n");
            $ar_info = pathinfo($image);
            pre_print($ar_info);
            if($ar_info['extension'] == 'png'){
                $filename = basename($image,".png");
                $t_file = __DIR__."/../images/".$filename.".png";
                if(!copy($image, $t_file)){
                    fwrite($f,"ERROR COPY ".$image.' => '.$t_file."\n");
                    echo 'ERROR COPY '.$image.' > '. $t_file."\n";
                }
                ...
                fwrite($f,"REZ ".$filename.".jpeg\n");
                $rez[] = "/import/images/".$filename.".jpeg";
            }else{
                $t_file = __DIR__."/../images/".$ar_info['basename'];
                if(!copy($image, $t_file)) echo 'ERROR COPY JPG '.$image.' > '. $t_file."\n";
                $rez[] = "/import/images/".$ar_info['basename'];
            }
        }
        fclose($f);
        print_r($rez);
